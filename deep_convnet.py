...
class DeepConvNet:
    ...
    def __init__(self, input_dim=(1, 28, 28),
        ...
        # enumerate()是一个内置函数，用于将一个可遍历的数据对象（如列表、元组或字符串）组合为一个索引序列，
        # 同时列出数据和数据下标。这样，你就可以在迭代时同时获取元素的值和它的索引。  （索引值，数据）
        ...
        self.layers.append(Affine(self.params['W8'], self.params['b8']))            #18
        self.layers.append(Dropout(0.5))
    ...
    def load_params(self, file_name="params.pkl"):
        ...
        # enumerate 是一个Python内置函数，用于将一个可遍历的数据对象（如列表、元组或字符串）组合为一个索引序列，同时列出数据和数据下标。
        for i, layer_idx in enumerate((0, 2, 5, 7, 10, 12, 15, 18)):
